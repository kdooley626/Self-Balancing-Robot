...
# State space vector: [theta, theta_dot]T
A = np.array([[0,1],[-c*M*g*L*(M+M_w+I_w/r**2),0]])
B = np.array([[0], [c*(M+M_w+I_w/r**2+M*L/r)]])
## Assess controllability of state space
control_matrix = np.column_stack((B, A@B))
control_rank = np.linalg.matrix_rank(control_matrix)
print(f"Control rank: {control_rank}")
